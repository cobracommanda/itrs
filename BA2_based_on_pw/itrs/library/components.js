  ...
  switch(true){  
    ...
    case (!!image_object && !this.zoomed_in && !this.has_annos):
      ...
      if(image_object.data_pages == ''){
        image_object.data_pages = !!figcaption.match(/(page|AR|p|pg)s?\.?(\s|-|_)*(&nbsp;)?\d+/ig) ? figcaption.match(/(page|AR|p|pg)s?\.?(\s|-|_)*(&nbsp;)?\d+/ig)[0].match(/\d+/)[0] : '';        
      }      
      if(image_object.data_pages == ''){
...
function process_header_paragraphs_in_lesson_florida(target_page) //new! forked from previous, pass it the first page
{
  var text = '';
  ...
  for(var t = 0; t < tfs.length; t++)
  {
    ...
    {
      // alert(tfs[t].itemLayer.name);
      if(!!tfs[t].paragraphs.length > 0 && tfs[t].itemLayer.name.match(/fl/i))
      {
...
/*
function has_el_icon(object){
  var result = false;
  var groups = object.groups;
  var textFrames = object.textFrames;
  var has_textFrame = textFrames.length > 0 ? true : false;
  var has_group = groups.length > 0 ? true : false;
  switch(true){
    case has_textFrame: 
      result = !!textFrames[0].contents.match(/EL/) ? true : false; 
    break;
    case has_group: 
      result = has_el_icon(groups[0]);
    break;
    default:
  }
  return result;
...
function process_words(paragraph){
  ...
  for(var x = 0; x < l; x++){    
    var word = words[x];        
    var contents = word.contents.trim();    
    var appliedParagraphStyle = word.appliedParagraphStyle.name;
    var appliedCharacterStyle = word.appliedCharacterStyle.name;
    var fontStyle = word.fontStyle;
    var underline = word.underline;
    var colorname = word.fillColor.name;
    var strike = word.strikeThru;    
    var teacher_talk = !!colorname.match(/tchrtalk|Text Structure|Benchmark\s*Blue\s*100C\s*87M|Lesson_Tchr-tlk-nospace|BEC blue/ig) || !!appliedCharacterStyle.match(/teacher_talk/i) || !!appliedParagraphStyle.match(/Lesson_Tchr-tlk/i) ? true : false;    
    var has_class = ( teacher_talk ) ? true : false;
    var _class = teacher_talk ? 'teacher_talk' : '';    
    var highlight = !!appliedParagraphStyle.match(/wg_c-hd_table/i) ? true : false;    

    var has_style = false;
    if(!word.fillColor.name.match(/none|black/ig)){
      has_style = true;
      if(!!word.fillColor){
        var color = get_hex_color(word.fillColor);
        var style = ' style="color:'+color+'"';
      }
    }

    if(process_as_characters(word)){
      text += process_characters(word);
    }else{
      var bold = !!fontStyle.match(/black|bold|demi|medium\scondensed|heavy/ig) ? true : false;
      var italic = !!fontStyle.match(/italic|oblique/ig) ? true : false;
      
      text += bold ? '<strong>' : '';
      text += italic ? '<em>' : '';
      text += underline ? '<u'+( highlight ? ' class="highlight"' : '' )+( has_style ? style : '')+'>' : '';
      text += strike ? '<s'+( has_style ? style : '')+'>' : '';
      // text += teacher_talk ? '<span class="teacher_talk">' : '';      
      text += has_style ? '<span'+ style +'>' : '';

      text += contents + ' ';

      text += has_style ? '</span><!-- end of '+color+' -->' : '';                
      // text += teacher_talk ? '</span><!-- end teacher_talk --> ' : '';
      text += strike ? '</s>' : '';
      text += underline ? '</u>' : '';
      text += italic ? '</em>' : '';
      text += bold ? '</strong>' : '';
  ...
  text = text.replace(/<\/u><u>/g, '');
  text = text.replace(/<\/s><s>/g, '');  
  // text = text.replace(/<\/span><!-- end teacher_talk --> <span class="teacher_talk">/g, '');
  text = text.replace(/<(\w|\s|=|")*>\s+<\/\w+>(<!--(\w|\s)*-->)?/g, ' ');
  text = text.replace(/<\/span><!-- end of (#?\w+) --><span style="color:(#?\w+)">/g, ''); 
  
  return text;
}

function process_as_characters(object){
  var contents = object.contents;
  var characters = object.characters.everyItem().getElements();
  var result = !!contents.match(/\W/) ? true : false;
...
function process_characters(object){
  var text = [];
  var characters = object.characters.everyItem().getElements();  
  var word = object.constructor.name == 'Word' ? true : false;
  var paragraph = object.constructor.name == 'Paragraph' ? true : false;

  characters.forEach(function(character, x){
    var contents = character.texts[0].contents;     
    contents = contents.replace(/\u0007/ig, '');
    contents = contents.replace(/\u2013/ig, '&nd;');
    contents = contents.replace(/\u2014/ig, '&md;');    
    contents = contents.replace(/\u0009/ig, '&emsp;');   
    contents = contents.replace(/(\u00af)/ig, '<span class="macron">$1</span>');
    var colorname = character.fillColor.name;
    var paragraphStyle = character.appliedParagraphStyle.name;
    var characterStyle = character.appliedCharacterStyle.name;
    contents = !!paragraphStyle.match(/caption/ig) ? contents.replace(/\u000A/ig, '<br>') : contents;
    
    var underline = character.underline;
    var strike = character.strikeThru;
    var teacher_talk = !!colorname.match(/tchrtalk|Text Structure|Benchmark\s*Blue\s*100C\s*87M|Lesson_Tchr-tlk-nospace|BEC blue/ig) || !!characterStyle.match(/teacher_talk/i) || !!paragraphStyle.match(/Lesson_Tchr-tlk/i) ? true : false;    
    var has_class = ( teacher_talk ) ? true : false;
    // var _class = teacher_talk ? 'teacher_talk' : '';
    var highlight = !!paragraphStyle.match(/wg_c-hd_table/i) ? true : false;

    var has_style = false;
    if(!character.fillColor.name.match(/none|black/ig)){
      has_style = true;
      if(!!character.fillColor){
        var color = get_hex_color(character.fillColor);
        var style = ' style="color:'+color+'"';
      }
    }

    var bold = !!character.fontStyle.match(/black|bold|demi|medium\scondensed|heavy/ig) ? true : false;
    var italic = !!character.fontStyle.match(/italic|oblique/ig) ? true : false;    
    ...
    text += (x == characters.length-1) ? contents+' ' : contents;
...
function build_tables(component, tables){    
  var text = '';
  tables.forEach(function(table){
    text += component.build_table(table);
  });   
  return text;
}
    ...
    if(row){      
      ...
      cells.forEach(function(cell){
        if(cell){                   
          ...
          text += '<td'+rowspan+colspan+' height="'+(Math.round(cell.height)*1.7)+'" width="'+column_width+'%" valign="'+valign+'"'+cell_style+'>\r';
...
function get_cell_style(cell){
  var cell_style = false;      
  var cell_color = get_cell_color(cell.fillColor);  
  var fillTint = !!cell_color ? cell.fillTint : 100;
  var background_color = (!!cell_color ? ( fillTint < 100 ? get_rgba(cell.fillColor, fillTint) : cell_color ) : '');
  var top_stroke_color = '';
  var top_stroke_weight = '';
  var bottom_stroke_color = '';
  var bottom_stroke_weight = '';
  var left_stroke_color = '';
  var left_stroke_weight = '';
  var right_stroke_color = '';
  var right_stroke_weight = '';
  
  if(cell.topEdgeStrokeWeight > 0){
    top_stroke_color = (cell.topEdgeStrokeWeight < 1) ? get_rgba(cell.topEdgeStrokeColor, cell.topEdgeStrokeWeight*100) : get_hex_color(cell.topEdgeStrokeColor);
    top_stroke_weight = (cell.topEdgeStrokeWeight < 1) ? 1 : cell.topEdgeStrokeWeight;    
  }
  if(cell.bottomEdgeStrokeWeight > 0){
    bottom_stroke_color = (cell.bottomEdgeStrokeWeight < 1) ? get_rgba(cell.bottomEdgeStrokeColor, cell.bottomEdgeStrokeWeight*100) : get_hex_color(cell.bottomEdgeStrokeColor);
    bottom_stroke_weight = (cell.bottomEdgeStrokeWeight < 1) ? 1 : cell.bottomEdgeStrokeWeight;    
  }
  if(cell.leftEdgeStrokeWeight > 0){
    left_stroke_color = (cell.leftEdgeStrokeWeight < 1) ? get_rgba(cell.leftEdgeStrokeColor, cell.leftEdgeStrokeWeight*100) : get_hex_color(cell.leftEdgeStrokeColor);
    left_stroke_weight = (cell.leftEdgeStrokeWeight < 1) ? 1 : cell.leftEdgeStrokeWeight;    
  }
  if(cell.rightEdgeStrokeWeight > 0){
    right_stroke_color = (cell.rightEdgeStrokeWeight < 1) ? get_rgba(cell.rightEdgeStrokeColor, cell.rightEdgeStrokeWeight*100) : get_hex_color(cell.rightEdgeStrokeColor);
    right_stroke_weight = (cell.rightEdgeStrokeWeight < 1) ? 1 : cell.rightEdgeStrokeWeight;    
  }


  var border_top = (cell.topEdgeStrokeWeight > 0 ? 'border-top: '+top_stroke_weight+'px solid '+top_stroke_color+';' : '');
  var border_bottom = (cell.bottomEdgeStrokeWeight > 0 ? 'border-bottom: '+bottom_stroke_weight+'px solid '+bottom_stroke_color+';' : '');
  var border_left = (cell.leftEdgeStrokeWeight > 0 ? 'border-left: '+left_stroke_weight+'px solid '+left_stroke_color+';' : '');
  var border_right = (cell.rightEdgeStrokeWeight > 0 ? 'border-right: '+right_stroke_weight+'px solid '+right_stroke_color+';' : '');

  var has_border = (cell.topEdgeStrokeWeight > 0) ? true : false;
  var same_stroke_color = (top_stroke_color == bottom_stroke_color && top_stroke_color == left_stroke_color && top_stroke_color == right_stroke_color) ? true : false;
  var same_stroke_weight = (top_stroke_weight == bottom_stroke_weight && top_stroke_weight == left_stroke_weight && top_stroke_weight == right_stroke_weight) ? true : false;

  var border = (same_stroke_color && same_stroke_weight && has_border) ? 'border: '+top_stroke_weight+'px solid '+top_stroke_color+';' : border_top+border_bottom+border_left+border_right;
  var background_color = (!!cell_color ? 'background-color: '+ background_color+';' : '');

  if(border_top || border_bottom || border_left || border_right || background_color){
    cell_style = border+background_color;
...
function get_cell_color(color){  
  if(!!color.name.match(/none/i)){return false;}
  var cell_color = false;
  var color_name = color.name.replace(/\W/g, '');    
  switch(true){
    case color.constructor.name == 'Tint':      
      cell_color = get_rgba(color.baseColor, color.tintValue);
    break;
    default: cell_color = get_hex_color(color);
  }
  return cell_color;
}

function get_rgba(color, fillTint){
  var alpha = (fillTint/100 < 0) ? 1 : fillTint/100;
  if(!!color.name.match(/black/i)){
    if(fillTint < 100){
  ...
  if(!!color.name.match(/paper/i)){return 'white';}
  color.space = ColorSpace.RGB;
  var red_value = parseInt(color.colorValue[0]);
  var green_value = parseInt(color.colorValue[1]);
  var blue_value = parseInt(color.colorValue[2]);
  
  return 'rgba('+red_value+','+green_value+','+blue_value+','+alpha+')';
}

function get_hex_color(color){
  if(!!color.name.match(/black/i)){return 'black';}
  if(!!color.name.match(/paper/i)){return 'white';}
  var result = '';
  try{
    color.space = ColorSpace.RGB;    
    result = rgb_to_hex(color.colorValue);
  }catch(e){
    if(!e.toString().match(RegExp('Error: This color cannot be edited.'))){
      log(e);
    }
    result = rgb_to_hex(color.colorValue);
  }
...
function process_groups(groups, object){  
  var text = '';
  var l = groups.length;
  for(var x = 0; x < l; x++){
    var group = groups[x];
    text += !!object ? export_and_create_diagram(group, object) : export_and_create_diagram(group);    
  }
  return text;
}
...
function process_rectangles(rectangles){
  ...
  for(var x = 0; x < l; x++){
    ...
    if(rectangle.allGraphics.length > 0){
      ...
      text += '<div class="checkbox"></div>\r';
    }   
  }  
  return text;
}

function export_and_create_diagram(group, object){  
  var image_link = create_image_file(group, current_package_folder, current_images_path);
  var width = 100;
  if(!!object){
    if(object.constructor.name == 'TextFrame'){      
      var textFrame_width = object.geometricBounds[3] - object.geometricBounds[1];
      var group_width = group.geometricBounds[3] - group.geometricBounds[1];
      var width =  Math.round((group_width/textFrame_width)*100);  
      width = width >= 80 ? 100 : width;
    }
  }
  return '<figure class="diagram" style="width:'+width+'%"><img src="'+image_link+'" alt=""></figure>';    
}
 
function resize_image(object, factor){  
  var anchors = AnchorPoint.TOP_LEFT_ANCHOR;
  if(object.constructor.name == 'Image' || object.constructor.name == 'PDF'){return;}
  if(object.constructor.name != 'Group'){
    object.resize(BoundingBoxLimits.GEOMETRIC_PATH_BOUNDS, anchors, ResizeMethods.MULTIPLYING_CURRENT_DIMENSIONS_BY, [factor, factor]);
    if(object.allGraphics.length > 0){
      object.allGraphics.forEach(function(image){
        image.resize(BoundingBoxLimits.GEOMETRIC_PATH_BOUNDS, anchors, ResizeMethods.MULTIPLYING_CURRENT_DIMENSIONS_BY, [factor, factor]);    
      });
    }
  }
  if(object.pageItems.length > 0){
    var items = object.pageItems.everyItem().getElements();
    items.forEach(function(item){
      resize_image(item, factor);
    });
  }
  object.fit(FitOptions.CONTENT_TO_FRAME);
}
*/ 
...
function ungroup_all(object)
{
  ...
  // {
  //   //this makes sure that groups with tilted items get preserved and labeled to be made into a figure (doesn't include items rotated in right angles)
  //   var okToUngroup = true;
  //   for(var i = 0; i < object.groups[g].pageItems.length; i++)
  //   {
  //     if(object.groups[g].pageItems[i].rotationAngle % 90 != 0)
  //     {
  //       object.groups[g].label = 'figure_group';
  //       okToUngroup = false;
  //     }
  //   }
  //   if(okToUngroup)
  //   {
  //     object.groups[g].ungroup();
  //   }
  // }
...
function findFileNameWW(fn_ycode, fn_grade, fn_unit, fn_week, fn_day, fn_lesson, fn_grep)
{
  ...
  // $.evalFile(app.activeScript.parent.parent.fsName+'/nav/filenames.js');
  $.evalFile(itrs_root_path+'/data/filenames.js');

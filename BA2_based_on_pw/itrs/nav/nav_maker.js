...
Navigation.prototype.make_unit_dropdown = function(){
	var text = '';	
	var unit_display = this.unit == 0 ? 'Launch' : 'Unit '+this.unit;
	...
	for(var x = 1; x < 11; x++){ //skip unit 0
		var unit = x;
		var product = get_product(this.grade, unit, region);
		var link = '';
		switch(unit){
		...
		text += '<a class="dropdown-item'+(unit == this.unit ? ' active' : '')+'" data-type="'+(unit == this.unit ? 'internal' : 'itrs')+'" data-code="'+product.xcode+'" data-extLink="'+link+'" href="'+link+'">'+(unit == 0 ? 'Launch' : 'Unit '+unit)+'</a>\r';
	}
	text += '</div>\r';
	text += '</div>\r';
	return text;
...
Navigation.prototype.make_print_button = function(classname){	
	var text = '';	
	// text += '<div class="print_button '+classname+'"><p><a data-code="'+current_package.dmv+'" data-type="html" data-extLink="javascript" href="javascript">Printable Version <span class="nav-icon-external-link"></span></a></p></div>\r'; //the old one, don't use
	// text += '<div class="print_button '+classname+'"><p><a href="/pdfs/print.pdf" target="_blank">Printable Version <span class="nav-icon-external-link"></span></a></p></div>\r'; //the default one
	text += '<div class="print_button '+classname+'"><!--<p><a href="/pdfs/print.pdf" target="_blank">Printable Version <span class="nav-icon-external-link"></span></a></p>--></div>\r'; //commented-out, don't use this most of the time
...
Navigation.prototype.make_card = function(card){
	...
	switch(true){
		...
		case (!!this.html_file_path.match(/overview/ig) && card.folder_name == 'overview'):
		case (!!this.html_file_path.match(/unit_resources/ig) && card.folder_name == 'unit_resources'):
		case (!!this.html_file_path.match(/week1/ig) && card.folder_name == 'week_1'):
		case (!!this.html_file_path.match(/week2/ig) && card.folder_name == 'week_2'):
		case (!!this.html_file_path.match(/week3/ig) && card.folder_name == 'week_3'):
		case (!!this.html_file_path.match(/additional_resources/ig) && card.folder_name == 'additional_resources'):
			show = ' show';
		break;
	}

	var nav_week = 1;
	if(card.folder_name == 'week_2') nav_week = 2;
	else if(card.folder_name == 'week_3') nav_week = 3;
	...
	if(card.folder_name.match(/week/))
	{

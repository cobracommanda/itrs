...
Header.prototype.toHTML = function(){
	...
	text += '<h3 class="whole_group_title">Toda la clase</h3>\r';
	text += '<h2>'+header+'</h2>\r';
	if(region == 'texas'){
	...
	if(!!collect_headers){
		timer_off = true;
		log('type: '+current_lesson_type+'\t'+current_package.grade+current_package.unit+'\t'+( current_lesson_type == 'ira' ? '<em>'+header.applyHTMLCode()+'</em>' : '<strong>D&iacute;a '+current_day+'</strong>: '+header.applyHTMLCode() ) );
		timer_off = false;
	}
	return text;
};
...
Generic.prototype.process_paragraph = function(paragraph){	
  ...
  var seld_header = ( header && !!condensed_contents.match(/Apoy(opara|ar)eldesarrollo(delidiomaespaol|delalenguaespaola)/i) ) ? true : false;
  var share_and_reflect_header = (h3 && this.title == 'Compartiryreflexionar') ? true : false;
  var bridge_to_transfer_header = (h3 && this.title == 'Puenteparatransferir') ? true : false;
  var has_icon = paragraph.groups.length > 0 ? true : false;
  var look_for_icon = has_icon ? ( (paragraph.groups[0].textFrames.length > 0) ? ( !!paragraph.groups[0].textFrames[0].contents.match(/pautas(\r|\n)*a\sseguir/i) ? true : false ) : false ) : false;  
  ...
  var left_indent = (!bullet && paragraph.leftIndent > 0) ? true : false;
	...
	switch(true){
		...
		case share_and_reflect_header: text += '<h3 class="whole_group_title">Toda la clase</h3>\r<h3>'+header_contents+'</h3>\r'; break;
...
function get_notes_textFrame(page){
	var result = get_textFrame_with_header(page, 'tomenotasobservacionales');	
...
FormativeAssessment.prototype.toHTML = function(){
	...
	text += '<div><h3><span class="brw-look-fors-icon"></span>Evaluaci&oacute;n formativa</h3></div>	\r';
	text += '<div class="row">\r';

...
LessonComponent.prototype.pour = function(week_value, day_value, type){
	...
	if(!!current_package.grade.match(/[k1]/i))
	{
		...
		// {
			...
			if(week==1)
			{
				filename = findFileNameWW(current_package.ycode, current_package.grade, current_package.unit, week, 1, 5, false);
				self.pour_file(filename);
			}
...
Lesson.prototype.toHTML = function(){
   ...
   {	
	...
	text = text.replace(/<strong>Comprobar/ig, '<strong><span class="icon-check"></span>Comprobar');
    ...
    text = text.replace(/<h5 class="blue">\s*Materiales(\s+adicionales)/, '<h5 class="blue"><a data-code="'+bankxcode+'" data-type="html" data-extLink="javascript" href="javascript">Materiales$1 <span class="nav-icon-external-link"></span></a>');
    text = text.replace(/Materialesundefined/, 'Materiales');
...
function header(textFrame, full_width){
  try
  {
	  ...
	  var msl_icon_header =  !!textFrame.paragraphs[0].rectangles.length > 0 && !!textFrame.paragraphs[0].rectangles[0].allGraphics[0].itemLink.name.match(/MultiSensoryLrngLogo|Aprendizaje_multisensorial/i) ? true : false;
      var mslTextHeader = msl_icon_header ? '&nbsp;<img class="img-fluid" style="height: 1em" src="/images/shared/Aprendizaje_multisensorial_Logo.png">' : '';
	  if(!!full_width){
	    text += '<div class="col-md-12">\r';
	  }else{
	  ...
	  if(current_grade.match(/[k12]/i))
	  	text += '<h2 class="day'+current_day+'-clr">'+contents.replace(/(\(\s*\d+)/, '<small>$1').replace(/(min.*?\))/i, '$1 <span class="standards">')+'</span></small>'+mslTextHeader+'</h2>';
	  else
	  	text += '<h2 class="week'+current_week+'-clr">'+contents.replace(/(\(\s*\d+)/, '<small>$1').replace(/(min.*?\))/i, '$1 <span class="standards">')+'</span></small>'+mslTextHeader+'</h2>';
...
function corrective_feedback_section(textFrame){
  var text = '';
  text += '<div class="corrective mt-4 mb-4">\r';
...
function mentor_writing_prompt_section(textFrame){  
  ...
  text += '</div>\r';
  text += '</div>\r';
  text = text.replace(/<p>.*?Pauta\smodelo\sde\sescritura.*?<\/p>/i, '').replace(/<p>Aseg.rate de incluir:?\s*?<\/p>/i, '<b>Aseg&uacute;rate de incluir:<\/b>\r<ul>');
...
function student_writing_prompt_section(textFrame){  
  ...
  text += '</div>\r';
  text += '</div>\r';
  text = text.replace(/<p>.*?Pauta\sde\sescritura\sdel\sestudiante.*?<\/p>/i, '').replace(/<p>Aseg.rate de incluir:?\s*?<\/p>/i, '<b>Aseg&uacute;rate de incluir:<\/b>\r<ul>');
  if(!text.match(/<ul>/)) text = text.replace(/<\/ul>/, '');
  return text;
}
...
//   text += '<img class="img-fluid" style="height: 50px" src="/images/shared/Aprendizaje_multisensorial_Logo.png">\r';
...
function msl_section(textFrame){
  ...
  try{ text += '<h5>'+textFrame.paragraphs[0].contents+'</h5>\r';		}catch(e){ text = text;}
...
function standards_section(textFrame){								//no updated version
  var text = '';
...
function ield_section(textFrame, msl_ield){
  ...
  var mslText = msl_ield ? '&nbsp;<img class="img-fluid" style="height: 2em" src="/images/shared/Aprendizaje_multisensorial_Logo.png">' : ''; //different, 2em
  // text += '<h5><span class="icon-ield"></span> '+getTerm('integrated_ield', language).plain+'</h5>\r';
  text += '<h5><span class="brw-ae"></span> '+textFrame.paragraphs[0].contents+mslText+'</h5>\r';
  if(isPhonicsLesson)
  {
  	text += '<div class="fluid-container">\r';
	text += '<div class="row">\r';
  ...
  text += process_paragraphs(textFrame);
  text = text.replace(/<p>(<strong>)?Apoyo (moderado|sustancial) (<\/strong>)?(.*?)<\/p>/ig, '<h4><strong>Apoyo $2 </strong>$4</h4>');
  text = text.replace(/<h4>Apoyo minimo <\/strong>/i, '<h4><strong>Apoyo minimo </strong>');
  if(isPhonicsLesson)
  {
  	// text = text.replace(/<\/ul>(\r|\n)<h4/g, '</ul>\r</div>\r<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">\r<ol>\r<h4');
	text = text.replace(/<h4>(<strong>)?Apoyo (moderado|sustancial) (<\/strong>)?/g, '</div>\r<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">\r<h4><strong>Apoyo $2 </strong>');
	text += '</div>\r';
	text += '</div>\r';
	text += '</div>\r';
  }
...
function buildreflectwrite_section(textFrame){
  ...
  text += '<h5 class="brw"><span>Ampliar</span><span>Reflexionar</span><span>Escribir</span></h5>\r';	//NOT IN TERMS
  text += process_paragraphs(textFrame);
  text += '</div>\r';  
  return text;
}
...
function formative_assessment_section(textFrame){
	...
	{
		case 'k': text += '<a href="javascript" data-extLink="javascript" data-code="X64623" data-type="html">\r'; break;
		case '1': text += '<a href="javascript" data-extLink="javascript" data-code="X64624" data-type="html">\r'; break;
		case '2': text += '<a href="javascript" data-extLink="javascript" data-code="X64625" data-type="html">\r'; break;
		case '3': text += '<a href="javascript" data-extLink="javascript" data-code="X64626" data-type="html">\r'; break;
		case '4': text += '<a href="javascript" data-extLink="javascript" data-code="X64627" data-type="html">\r'; break;
		case '5': text += '<a href="javascript" data-extLink="javascript" data-code="X64628" data-type="html">\r'; break;
		case '6': text += '<a href="javascript" data-extLink="javascript" data-code="X64629" data-type="html">\r'; break;
	}
	text += '<img class="img-thumbnail" src="/images/shared/assessments_grade'+current_grade.toLowerCase()+'_span.jpg" alt="">\r';
	text += '</a>\r';
	text += '</div>\r';
  ...
  for(var x = 0; x < l; x++){
    ...
    if(start){
      ...
      switch(true){        
        ...
        case pointSize == 14: 
          if(ield_icon){
            text += '<h3>'+contents.replace(/\n|\r/g, '')+'<span class="brw-ae"></span></h3>\r'; 
          }else{
            text += '<h3>'+contents.replace(/\n|\r/g, '')+'</h3>\r'; 
          }        
        break;
...
function process_textFrame(textFrame){   
  ...
  {	
	  ...
	  var msl_box = !firstParagraphContents.match(/\w/) && firstParagraph.rectangles.length > 0  ? ( !!firstParagraph.rectangles[0].allGraphics[0].itemLink.name.match(/MultiSensoryLrngLogo|Aprendizaje_multisensorial/i) ? true : false ) : false;
	  var msl_ield = firstParagraph.rectangles.length > 0 ? ( !!firstParagraph.rectangles[0].allGraphics[0].itemLink.name.match(/MultiSensoryLrngLogo|Aprendizaje_multisensorial/i) ? true : false ) : false;
...
function process_paragraphs(object){  
  ...
  if(!!paragraphs)
  {
	  ...
	  for(var x = 0; x < l; x++){
	    ...
	    var msl_icon = false;
	    try{msl_icon = (a_head || h3) && has_rectangle ? ( !!paragraph.rectangles[0].allGraphics[0].itemLink.name.match(/MultiSensoryLrngLogo|Aprendizaje_multisensorial/i) ? true : false ) : false;}catch(e){}
	    var msl_icon_only = false;
	    try{msl_icon_only = empty && has_rectangle ? ( !!paragraph.rectangles[0].allGraphics[0].itemLink.name.match(/MultiSensoryLrngLogo|Aprendizaje_multisensorial/i) ? true : false ) : false;}catch(e){}
	    var msl_box = false;
	    try{msl_box = empty && has_textFrame && !!current_grade.match(/[k12]/i) ? (!!paragraph.textFrames[0].paragraphs[0].rectangles[0].allGraphics[0].itemLink.name.match(/MultiSensoryLrngLogo|Aprendizaje_multisensorial/i) ? true : false) : false;}catch(e){}
	    var music_logo = false;
	    try{music_logo = has_rectangle ? ( !!paragraph.rectangles[0].allGraphics[0].itemLink.name.match(/Logo 123|Bec_Music_Video_Icon|HeidiSongs_Logos/i) ? true : false ) : false;}catch(e){} //intelligent: it replaces heidisongs logos, wow!
	    var ield_icon = has_textFrame ? ( !!paragraph.textFrames[0].contents.match(getTerm('ield', language).regex) ? true : false ) : false;    
	    var essential_question = !!paragraphStyle.match(/Essential_question_Body-txt/i) ? true : false;
	    var speech_bubble_icon = !!contents.match(/conversaci.n\sconstructiva/i) ? true : false; //has_rectangle ? ( !!paragraph.rectangles[0].allGraphics[0].itemLink.name.match(/new_conversation/i)
	    var video_play_icon = !!contents.match(/ver\smultimedia/i) ? true : false; //has_rectangle ? ( !!paragraph.rectangles[0].allGraphics[0].itemLink.name.match(/(playvideo_icon|asset)/i) ? true : false ) : false;
	    ...
	    // var has_epocketchart_or_cards = empty && has_textFrame && !!current_grade.match(/[k12]/i)  ? (!!paragraph.textFrames[0].paragraphs[0].appliedParagraphStyle.name.match(/(e-pocket|word_clues)/i) ? true : false) : false;


      try{
        var has_epocketchart_or_cards = empty && has_textFrame && !!current_grade.match(/[k12]/i)  ? (!!paragraph.textFrames[0].paragraphs[0].appliedParagraphStyle.name.match(/(e-pocket|word_clues)/i) ? true : false) : false;
      }catch(e){
      var has_epocketchart_or_cards = false;
      }
	    ...
	    switch(true){
	      ...
	      case ((ol_opened && !bullet) || listType == ListType.BULLET_LIST): ol_opened = false; text += '</ol>\r'; break;
	    }
	    switch(true){
	    ...
	    var mslText = msl_icon ? '&nbsp;<img class="img-fluid" style="height: 1em" src="/images/shared/Aprendizaje_multisensorial_Logo.png">' : ''; //can be on anything
	    switch(true){ 
	      ...
	      break;     
	      case has_rectangle && empty && music_logo: text += '<p><img class="img-fluid" style="height: 125px" src="/images/shared/logo_123.jpg"></p>\r'; break;
	      case has_rectangle && empty: text += process_rectangles(paragraph.rectangles); break;
	      case has_epocketchart_or_cards: text += process_textframes_as_diagrams(paragraph.textFrames); break;
	      ...
	      case h3: // || (red && pointSize == 14))
	        ...
	        // log(contents);
	        var ieldText = ield_icon ? '&nbsp;<span class="brw-ae"></span>' : ''; //can be on anything
	        switch(true){
	      ...
	      default: 
	      	if(music_logo) text += '<p><img class="img-fluid" style="height: 125px" src="/images/shared/logo_123.png"></p>\r';
	      	text += '<p>'+(accesskey_icon?'<span class="brw-icon-accesskey-sp"></span>':'')+process_words(paragraph)+'</p>\r';
	  ...
	  // text = text.replace(/<p>(<strong>)?<span class="blue">Pr.ctica.+?sindependiente\/\s?Partner.+?Work\s?<\/span>(<\/strong>)?<\/p>/g, '<h3>Independent Practice/Partner Work</h3>');
	  text = text.replace(/&(m|n)d;/ig, '&$1dash;');    
	  return text;
  }
}
...
function process_textframes_as_diagrams(frames){
  var text = '';
  var l = frames.length;
  for(var x = 0; x < l; x++){
    var frame = frames[x];
    text += export_and_create_diagram(frame);  
  }
  return text;
}
...
function export_and_create_diagram(group){
  var image_name = '';
  // var named = false;
  // if(group instanceof TextFrame)
  // {
  // 	if(group.paragraphs.length > 0)
  // 	{
  // 		if(group.paragraphs[0].characters.length > 0)
  // 		{
  // 			image_name = 'figure'+group.paragraphs[0].contents+'.png'; 
  // 		}
  // 	}
  // }
  // if(!named) 
  	image_name = 'figure'+group.id+'.png';  
...
function build_tables(tables){    
	...
	for(var x = 0; x < l; x++)
	{
		...
		if(keepGoingTables)
		{
			...
			switch(true)
			{
				case small_group_table: text += create_small_group_table(table); break;
...
function create_small_group_table(table)
{
	var text = '';
    ...
    text += '<div><span class="icon-smallgroup"></span></div>\r';
    text += '<div class="col">\r'; 
    text += '<h5 class="blue">'+table.cells[0].paragraphs[0].contents.toUpperCase().replace(/^\W+(\w)/, '$1').replace(/-/, '&ndash;').replace(/\((.+?)\)/i, '<small><strong>($1)</strong></small>')+'</h5>\r';
    text += '</div>\r';
    text += '</div>\r';
    
	for(var tp = 1; tp < table.cells[0].paragraphs.length; tp++)
	{
		if(table.cells[0].paragraphs[tp].contents.match(/:/))
			text += '<p>' + table.cells[0].paragraphs[tp].contents.replace(/^(.+?:)/, '<strong>$1</strong>').replace(/(:<\/strong>)(.+)$/, '$1<em>$2</em>') + '</p>\r';
		else
			text += '<p>' + table.cells[0].paragraphs[tp].contents + '</p>\r';
	}
...
function phonics_table_rectangle_images_handler(cell, rectangles, index){  
  ...
  for(var x = 0; x < l; x++){
    ...
    try
    {
	    if(!!caption.match(/\w/))
		{
			if(!!caption.match(/\d/) && datatype == 'ebook') 
			{
				if(caption.match(/(page|AR|p|pg|p[^\w\s]g)(ina)?s?\.?(\s|-|_)*(&nbsp;)?\d+/i))
					datapages = caption.match(/(page|AR|p|pg|p[^\w\s]g)(ina)?s?\.?(\s|-|_)*(&nbsp;)?\d+/i)[0].match(/\d+/)[0]; //p\w+\.?\s\d+
			}
		}
	}
	catch(e){}
...
function build_table(table, classname, special_type){
  ...
  switch(special_type)
  {
  	...
  	case 'brw': 
  		text += '<div class="blue-border-box mt-4 mb-4">\r';
		text += '<h5 class="brw"><span>Ampliar</span><span>Reflexionar</span><span>Escribir</span></h5>\r'; 
		break;
  	case 'small_group_table': 
		text += '<div class="small-group mt-4 mb-4">\r';
		text += '<div class="row">\r';

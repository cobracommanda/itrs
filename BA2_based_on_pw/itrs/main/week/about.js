...
function Introduction(filename){	
	...
	this.title;
	switch(true){
		case current_lesson_type == 'ira': this.page1_index = 0; this.page2_index = 1; this.title = 'ReadAlouds'; break;
		case current_lesson_type == 'sr' && !!current_package.grade.match(/[k12]/i): this.page1_index = 2; this.page2_index = 3; this.title = 'SharedReadings'; break;
		case current_lesson_type == 'rml' && !!current_package.grade.match(/[k12]/i): this.page1_index = 4; this.page2_index = 5; this.title = 'ReadingMiniLessons'; break;		
		case current_lesson_type == 'rml' && !!current_package.grade.match(/[345]/): this.page1_index = 2; this.page2_index = 3; this.title = 'ReadingMiniLessons'; break;
...
SkillsAtAGlance.prototype.process_paragraph = function(paragraph){	
  ...
  var unit_header = (!!condensed_contents.match(/unit\d+/i) && h2) ? true : false;  
	...
	switch(true){
		...
		case unit_header: text += '<h2 class="skills_at_a_glance"><span class="bold">Unit '+current_package.unit+'</span>'+contents.replace(/unit\s*\d+/i, '')+'</h2>\r'; break;

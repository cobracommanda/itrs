...
function GuideToSharedReadingLowerGrade(week){
	...
	(3).forEvery(function(number){
		if(!!week && (number+1) == week){
			...
			self.indd_filename = findFileNameWW(current_package.ycode, current_package.grade, current_package.unit, false, false, false, 'TextComplexity_SR');
			if(!self.indd_filename) self.indd_filename = findFileNameWW(current_package.ycode, current_package.grade, current_package.unit, false, false, false, 'TextComplexity_SharedReading');
...
GuideToSharedReadingUpperGrade.prototype.toHTML = function(){
	var text = '';
	text += '<h2>Gu&iacute;a de las Lecturas compartidas</h2>\r';
